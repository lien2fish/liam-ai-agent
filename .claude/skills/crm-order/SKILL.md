...
### 節慶檔期 vs 經常性（2026-09-11 建立，首檔＝2026中秋烤肉組）
**不另開表，同一份資料用兩個欄位區隔**——分兩處記，月報會漏算或重算。客戶名單也只有一份。

| 欄位 | 值 | 在哪些 DB |
|---|---|---|
| `販售類型` | 經常性／節慶限定 | 🐟 鑫海產品項表、🐟 鑫海產銷售、🧾 全品牌銷售紀錄 |
| `檔期` | `2026中秋`、`2027春節`…（經常性留空） | 同上 |

- **品項**：節慶組合寫在 `crm_unified/seafood_bundles.json`（不在 xlsx，否則同步會封存）。
  過季就從 JSON 刪掉，下次 `sync_inventory.py` 自動封存。⚠️ 這個 JSON 在公開 repo，**只放 DM 上公開的售價，不放進價**。
- **訂單**：`add_order.py` 一律填「經常性」。**節慶單要手動改成節慶限定＋檔期**
  （Notion 兩個銷售 DB 都要改；Numbers 沒有這欄，寫進備註「節慶限定｜檔期」）。
- **月報**：`monthly_report.py` 多一段「🥮 經常性／節慶」拆分，`販售類型` 空白視同經常性。
- **查看**：全品牌銷售紀錄的「🥮 2026中秋」檢視、品項表的「依販售類型」檢視。新檔期要自己加一個檢視。
- Numbers 銷售報表第 G／H 欄（毛利、毛利率）是**預先鋪好的公式**，寫新列時跳過這兩欄，不要寫死值。
