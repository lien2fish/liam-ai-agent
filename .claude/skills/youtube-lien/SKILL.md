...
### 發布節奏（2026-09-07 依實際排程更正）

- **Shorts 每兩天 18:00 一支**（連老闆與甜點頻道同節奏）
- 🔴 **長片與同主題的第一支 Shorts 同日發布**（Lien 2026-09-07 說明）。
  短片負責導流到長片，所以兩者同一天上線、不是錯開。
  ⚠️ **舊版寫「長片排在該 Shorts 前一天、不與 Shorts 同日發」是錯的**，
  2026-09-07 查頻道實際排程推翻：9/21 虎蝦長片＋「這隻蝦最貴的地方」同日、
  9/29 生蠔長片＋「生蠔開不開得了」同日，歷史上 7/24 崁仔頂、8/04 台南漁市、
  8/12 台灣鯛也都是同日配對。
- 排長片時**只動長片**，Shorts 的日期一律不改

⚠️ **算發布間隔時要先把長片排除。** 長片與 Shorts 同日，
把兩者混在同一條序列上算「隔幾天」，會把正常的每兩天誤判成「連續兩天發、節奏壞掉」
——2026-09-07 因此提出一份會拆散長片配對的「修正表」，差點照做。
**查排程一律連 `contentDetails.duration` 一起取，先分長片／Shorts 再算間隔。**

### 發布時間的實測（2026-09-07 拉 YouTube Analytics）
...
**18:00 已驗證，不要動。** 前 8 名有 7 支是 18 點發的。

星期別看起來有差（日 1,190／二 1,188／六 603），但**每個星期只有 3～6 支，
樣本不足以下結論**，一支爆或一支冷就翻盤。每兩天的排法本來就會讓星期輪動、長期抹平，
**不要為了這個去改節奏**。

🔴 **真正的槓桿是題材不是時間**：星期別差距約 2 倍（603～1,190），
題材類型差距約 10 倍（200～2,380）。揭弊／破除迷思是爆點，
單純的挑選知識落在 200～400。見 memory `project_channel_performance_pattern`。
