...
| Email 位址（2026-09-14）| `High@4.0`、`gsap@3.12` 這類版本號也算 Email，repo 裡 9 個檔案每次動到都跳 ⚠️ | 網域結尾必須是英文字母（`.com`／`.tw`）；結尾不用 `\b`，否則 Email 後面直接接中文字的會漏掉 |
...
- 🔴 **文案／敏感字串可能寫死在程式裡，只移資料檔擋不住。**
  2026-09-13 把提案旁白移出公開庫時，`script.md`／`scenes.json` 都處理了，
  但片頭主標、片尾 CTA、交付三卡的文字全部寫死在 `build.py` 的 HTML 模板裡，
  而產出的 `index.html`（本來就該 gitignore 的衍生檔）更是含全文。
  **驗證要用「招牌台詞」grep 全庫，不能只看檔名**：
  ```bash
  git diff --cached | grep -c "<只有那份文件才有的一句話>"     # 要 0
  git diff --cached | grep -c "<確定存在的東西>"                # 對照組，要 >0
  ```
  第一輪 grep 就是這樣抓到 `build.py` 還有一句漏網的。
...
- 🔴 **`checkout -B` 保留的是「未追蹤」檔案，「已追蹤」的會被刪光。**
  改寫歷史把某個目錄整包移出去之後，本機那個目錄裡**還在版控的檔案會一起消失**——
  包含你打算留下來的部分（2026-09-13 實際踩到：`proposal_video/` 只剩未追蹤的
  產出與素材，`build.py` 等十個工具檔全被刪）。
  **跟上遠端之前先 `cp -R` 整個目錄到備份區**，跟上之後再還原、重新 commit。
