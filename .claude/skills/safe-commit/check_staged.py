...
CONTENT_WARN = [
    ...
    # 網域結尾必須是英文字母（.com／.tw）。舊版 `[\w.]+` 會把 High@4.0、gsap@3.12 這種版本號
    # 當 Email，repo 裡 9 個檔案每次動到都跳 ⚠️（2026-09-14 收斂）。
    # 結尾用 (?![A-Za-z]) 不用 \b——\w 含中文，Email 後面直接接中文字時 \b 會漏掉。
    (r"[\w.+-]+@[\w-]+(?:\.[\w-]+)*\.[A-Za-z]{2,}(?![A-Za-z])", "含 Email 位址"),
    (r"統一?編號\s*[:：,]", "含統編欄位"),
]
