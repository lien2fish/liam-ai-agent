...
⚠️ **排程沒跑有兩種，別混為一談。** 兩者都是 GitHub 平台端問題，**不是設定錯誤，改 cron 沒有用**：

| | 誤點 | **整批被丟** |
|---|---|---|
| 樣子 | 該跑的還是跑了，只是晚 1.5~4 小時 | 某個時間窗內**一支都沒觸發**，那批任務直接消失，**GitHub 不會補跑** |
| 怎麼認 | 單支任務時間往後移，高頻的 `ig_comment_reply` 最明顯 | 拉全 repo 的 runs 看**時間軸有沒有空洞**——連每 5 分鐘的 `ig_comment_reply` 都斷掉就是它 |
| 處置 | 等，別動 | 手動 dispatch 補跑 |

**判準：壞掉會留 failure，被丟掉連 run 都不會生出來。**
所以「Actions 全綠但今天的報告沒出來」＝被丟掉，不是程式壞了。
先看「上一次成功是什麼時候」，**還沒到就急著修，會改壞沒壞的東西。**

查空洞用上面那條 `actions/runs?per_page=100`，一次掃全部任務最快。

**2026-08-27 實例**：UTC 08-26 23:32 → 08-27 04:58 共 5 小時 26 分空窗，
台灣 08:00（UTC 00:00）那批 9 支全部沒觸發，`ig_comment_reply` 這 5 小時也一次都沒跑。
所有 workflow 最後一次排程執行都停在前一天、**全部 success、零紅燈**。
同一支 `daily_post` 手動 dispatch 1 分 19 秒就跑完並成功發文——證明程式與金鑰都好好的。
...
|---|---|---|---|
| **今天完全沒有 run**（不是紅燈，是 Actions 裡根本找不到今天的紀錄） | GitHub 排程整批被丟，見上方對照表 | 手動 dispatch 補跑。**先確認今天本來該不該跑**（YouTube 自動影片只在週二/五/日，週四沒跑是正常的） | ✅ |
...
| | 任務 | 為什麼 |
|---|---|---|
| ✅ 直接補 | Gmail、保單、生日、扶輪生日、拜訪、**Token 到期檢查**、回購、漁獲、YT 留言／日報、市場日報 | 只寄信或寫報告，重跑無副作用；報告同日重跑會覆蓋，不會長出兩份 |
| ⛔ 不要補 | **IG+FB 發文、IG 限動預告** | 發出去 API 刪不掉（見文末）。先確認今天是不是已經發過——**手動 dispatch 過也算發過** |
| — 先確認 | YouTube 自動影片 | 只在週二/五/日製作，其他天沒跑是正常的 |

**優先補 `token_expiry_check`**：那是唯一會主動盯 IG／FB token 的東西，斷一天就少一天預警。
