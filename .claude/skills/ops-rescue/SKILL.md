...
⚠️ **2026-08-29 起「該跑沒跑」的判斷變嚴格了。** 計時搬到 Cloudflare Worker 之後，
觸發是準時的——**時間到了沒有 run，就是真的有問題**（Worker 掛了、`GITHUB_PAT` 失效、
或那支 workflow 自己壞了），不再是「等一下就會來」。
...
⚠️ **2026-08-29 起全部由 Cloudflare Worker `liam-scheduler` 計時**，workflow 檔裡沒有任何 cron。
排程對照表以 `scheduler_worker/worker.js` 的 `SCHEDULE` 為準（線上可直接查
`curl https://liam-scheduler.lien2fish.workers.dev/`），不要再去 workflow 檔裡找 cron。

| 時間 | 任務 | 有 commit？ |
|---|---|---|
| 每 30 分鐘 | IG 留言回覆 | — |
| 08:03（一）/ 08:13 | 營收衝刺週報／Gmail 清理＋新聞 | Gmail✓ |
| 08:17 / 08:23 / 08:27 | 保單到期／生日提醒／扶輪生日 | 保單✓ |
| 08:33 / 08:37 / 08:39 | YT 頻道日報／IG+FB 發文／YT 留言通知 | 日報✓ 留言✓ |
| 08:43 / 08:47 | 壽險拜訪／Token 到期檢查 | — |
| 08:53（一）/ 08:57（1 日） | AI 工作週報／Notion 月報 | — |
| 09:07 / 09:37 | 回購提醒／漁獲行情 | 行情✓ |
| 10:07 | YouTube 自動影片（只在週二、五、日製作） | — |
| 12:07 | 市場日報 | ✓ |
| 18:08 | IG 限動預告（接 18:00 的 Reels） | ✓ |
| 10:30 / 20:30 | Worker 自己回頭稽核有沒有漏跑，漏了推 LINE | — |
