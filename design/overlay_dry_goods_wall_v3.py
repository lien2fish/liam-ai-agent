#!/usr/bin/env python3
"""惜食廚房一樓主視覺牆 v3 — 卡片尺寸對齊「半張A4(A5橫式210x148mm)以上」

牆面實際尺寸：3.42m x 1.46m
版面：16欄 x 3列志工格(取40格) + 5張廚師卡
來源背景：/Users/lien/Downloads/Gemini_Generated_Image_qfba0aqfba0aqfba.png (1584x672)
輸出：design/output/dry_goods_wall_v3_final.png
"""
import os
...
GAP = 14
COLS = 16
ROWS = 3  # 志工格列數 (16x3=48, 取40)

TITLE_H = 110
CHEF_H = 320
VOL_H = 246

CONTENT_W = W - 2 * MARGIN  # = COLS*card_w + (COLS-1)*GAP
CARD_W = (CONTENT_W - (COLS - 1) * GAP) / COLS  # ≈ 176px ≈ 191mm (>A5寬)
...
# ---- 廚師卡片 (5張，每張寬度 = (CONTENT_W - 4*GAP)/5) ----
...
# ---- 志工卡片 (16欄 x 3列, 編號1~40, 第3列右側8格留白備用) ----
...
out_path = os.path.join(OUT_DIR, "dry_goods_wall_v3_final.png")
img.save(out_path)

mm_per_px = 3420 / W
print(f"已儲存：{out_path}　尺寸：{img.size}　比例：{W/H:.4f}（目標 {WALL_RATIO:.4f}）")
print(
    f"志工卡尺寸：{CARD_W*mm_per_px:.1f} x {VOL_H*mm_per_px:.1f} mm（半A4=148x210mm 或 210x148mm）"
)
