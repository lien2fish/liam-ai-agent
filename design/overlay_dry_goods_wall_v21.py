#!/usr/bin/env python3
"""志工牆 v21 = v20 版面改成 300×120cm（342×146 的 v20 見 overlay_dry_goods_wall_v20.py）。

比例由 2.3423 變 2.5，志工格 5 列→**4 列(10 欄＝40 格)**，格子改用撐滿高度的 1.27 比例
(不再是 A5 橫式)：格子的**水平範圍動不了**——左右側欄食材圖示與右下官網 QR 面板把可用寬
鎖在原本那段，硬撐到滿版會壓到它們，所以 vw 沿用 342 版的相對寬、只讓 vh 吃掉多出來的高度。

底圖來源＝dry_goods_wall_base_300x120.py 產的 v21 clean_base。以下沿用 v20：
...
WALL_W 指定輸出寬度(px，預設母檔 3150)。成品板面 300×120cm：
  3150px=27dpi(太低) / 8504px=72dpi(送印用)。文字與裱框是程式畫的→放大真的變銳利，
...
SRC = DESK + "/dry_goods_wall_v21_clean_base_300x120.png"
OUT = DESK + "/dry_goods_wall_v21_志工照片版_300x120" + (f"_{OUT_W}px" if OUT_W != BASE_W else "") + ".png"  # fmt: skip
...
VW_FRAC = 243.58 / 3148  # 342 版的志工格寬相對牆寬，換尺寸不動它才不會壓到側欄裝飾
...
def main():
    ...
    # 300×120 只放得下 40 格(342 版是 50)，超過會畫到牆外
    assert (
        len(photo_names) + len(noimg_names) <= 40
    ), f"志工 {len(photo_names) + len(noimg_names)} 位 > 40 格"
    ...
    VC, VR = 10, 4
    vh = (H - 2 * MARGIN - TITLE_H - GAP - GAP - (VR - 1) * GAP) / (VR + 1.0)
    vw = VW_FRAC * W  # 水平被側欄圖示與右下 QR 面板鎖死，沿用 342 版的格寬
    gw = VC * vw + (VC - 1) * GAP
    ...
    def card(box, head_h, head_fill):
        """白卡＋頂端色條＋灰照片框＋人形剪影。342 版這層是畫在底圖上的，
        300 版的底圖把格子區洗成米色了(列數變了對不上)，改由這裡依新版面重畫。"""
        x0, y0, x1, y1 = box
        d.rounded_rectangle(box, radius=12 * S, fill=WHITE)
        d.rounded_rectangle((x0, y0, x1, y0 + head_h), radius=12 * S, fill=head_fill)
        d.rectangle((x0, y0 + head_h - 12 * S, x1, y0 + head_h), fill=head_fill)
        pb = (x0 + 8 * S, y0 + head_h + 8 * S, x1 - 8 * S, y1 - 24 * S)
        d.rounded_rectangle(pb, radius=9 * S, fill=GRAY)
        pw, ph = pb[2] - pb[0], pb[3] - pb[1]
        cx, hr, hcy = (pb[0] + pb[2]) / 2, ph * 0.18, pb[1] + ph * 0.32
        d.ellipse((cx - hr, hcy - hr, cx + hr, hcy + hr), fill=SILH)
        d.rounded_rectangle(
            (cx - pw * 0.2, hcy + hr * 0.6, cx + pw * 0.2, pb[3] - ph * 0.08),
            radius=ph * 0.06,
            fill=SILH,
        )
        d.line((x0 + 14 * S, y1 - 12 * S, x1 - 14 * S, y1 - 12 * S), fill=(210, 200, 190), width=round(3 * S))  # fmt: skip
    ...
    # ===== 卡片底(含填不滿的空格) =====
    for i in range(VC * VR):
        card(slot(i), hh, AMBER)
    for k in range(5):
        x0 = vl + k * (vw + cgap)
        card((x0, chef_top, x0 + vw, chef_top + vh), hh_c, GREEN)
    ...
    for j, name in enumerate(noimg_names):
        ...
        ImageDraw.Draw(m).rounded_rectangle(
            (0, 0, pw, ph), radius=round(9 * S), fill=255
        )
    ...
    for k, (name, label, fname) in enumerate(CHEF_FROM_RIGHT):
        ...
        ctext(
            (x0, y0, x1, y0 + hh_c), txt, fit_font(txt, vw - 26 * S, cheff.size), WHITE
        )
