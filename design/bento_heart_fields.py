...
用法：python3 design/bento_heart_fields.py [--width-cm 30] [--dpi 300]
"""
...
def build(width_cm, dpi, outdir):
    ...
    heart = Image.new("RGBA", (target_w, target_h), HEART_RGB + (0,))
    ...
    for y, name in zip(rows, FIELDS):
        ...
        draw.rounded_rectangle(
            [line_x0, base - rule, line_x1, base],
            radius=rule // 2,
            fill=(255, 255, 255, 255),
        )
    ...
    print(f"愛心尺寸 {target_w}×{target_h}px @{dpi}dpi = {width_cm:g}×{target_h/dpi*2.54:.1f}cm")
    print(f"文字塊寬 {block_w}px（{block_w/dpi*2.54:.1f}cm）／字面 {font.size}px／底線 {line_x1-line_x0}px")
    for f, (lo, hi), y in zip(ROW_FRACS, avail, rows):
        print(f"  y={y} 可用 {lo}..{hi} 邊距 左{x_left-lo} 右{hi-line_x1}")
    print("輸出：", png, os.path.join(outdir, stem + "_預覽.png"), ai, sep="\n  ")
