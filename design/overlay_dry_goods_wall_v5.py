#!/usr/bin/env python3
"""惜食廚房一樓主視覺牆 v5 — 廚師4位 + 行政2位，志工格橫向半A4以上 (A5橫式 210x148mm 比例)

牆面實際尺寸：3.42m x 1.46m
版面：10欄 x 4列志工格(共40格，剛好用完) + 4張廚師卡 + 2張行政卡
來源背景：/Users/lien/Downloads/Gemini_Generated_Image_qfba0aqfba0aqfba.png (1584x672)
輸出：design/output/dry_goods_wall_v5_final.png
"""
import os
...
TOP_COUNT = 6  # 4廚師 + 2行政
top_w = (CONTENT_W - (TOP_COUNT - 1) * GAP) / TOP_COUNT
...
for i, (label, color) in enumerate(TOP_CARDS):
    x0 = MARGIN + i * (top_w + GAP)
    x1 = x0 + top_w
    y0 = chef_top
    y1 = chef_top + CHEF_H
    rounded_rect((x0, y0, x1, y1), radius=20, fill=WHITE)
    header_h = CHEF_H * 0.16
    rounded_rect((x0, y0, x1, y0 + header_h), radius=20, fill=color)
    d.rectangle((x0, y0 + header_h - 20, x1, y0 + header_h), fill=color)
    draw_centered_text((x0, y0, x1, y0 + header_h), label, chef_font, WHITE)
...
out_path = os.path.join(OUT_DIR, "dry_goods_wall_v5_final.png")
img.save(out_path)
...
print(f"廚師/行政卡尺寸：{top_w*mm_per_px:.1f} x {CHEF_H*mm_per_px:.1f} mm")
