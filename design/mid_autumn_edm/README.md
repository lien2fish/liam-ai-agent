...
原本規劃放訂購方式與截止／取貨日，2026-09-03 Lien 決定不放，
改成「限量 100 組　要搶要快」。這張圖會發在已經知道怎麼聯絡的通路
（LINE 群、IG），所以沒有 CTA 不影響下單。

⚠️ 「要搶要快」是促銷語氣，與 `seafood-brand` skill 的「高端克制、不喊促銷」
有拉扯——這是 Lien 對本檔期的決定，不是疏漏，**不要自作主張改回去**。
...
| 現流海味 | NT$1,299 | 活蝦 1 斤、砲管透抽 1 支、秋刀魚 ×4、鯖魚片 ×2 |
| 兩組合購 | NT$4,288 | 單買合計 4,598，省 310 |
...
- **羽化圖之間圓心距離要 > 兩者可見半徑之和**（可見直徑約為寬度的 0.68 倍），
  否則會糊成一團。改版面後用這條算一次再出圖，比事後看截圖省事。
- **拿掉品項就要拿掉對應的圖**，否則圖文對不上（2026-09-03 移除蛤蜊時一併刪了 `clam.jpg`）。
  圖數量變動後記得重排——四張的 2×2 少一角會歪，三張要改成錯落分布。
