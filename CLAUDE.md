...
## 惜食廚房三角牆活動相簿（2026-07-27 建立）
本專案**第一面異形（非矩形）輸出**。腳本 `design/tri_wall_savefood.py`，產物在 `惜食廚房輸出/三角牆_歷屆活動相簿/`。

| 項目 | 內容 |
|------|------|
| 尺寸 | 頂邊 295cm 水平／左邊 180cm 直立／**底斜邊 350cm**，尖角在**左下**。面積 2.6539 m² |
| 版面 | **滿版壓暗實景底圖**（暖色：木桌上的熱飯與蒸氣）＋頂部白色大標「一餐一飯 ‧ 都是疼惜」(228mm 字高，7px 深綠描邊) ＋ 之後疊 **10 格照片 4/3/2/1**，每格 43.9×25.9cm |
| 用法 | `--bg <素材>`（底圖，現行交付形態）／`--cells`（疊照片框校稿）／`--photos <dir>`（真照片，檔名前兩碼＝格號）／`--pdf`（100dpi DeviceCMYK 送印檔）／`--gen-bg <seed> --scene <table\|bento\|pantry\|volunteers>`（生底圖素材）／`--tint warm\|green` |
| 現況 | **底圖已定案**（暖色 table 版，素材存 `素材/底圖素材_table_seed20260727.png`），檢查全過；活動照片協會尚未提供 |

- **底圖走既有 12 面語彙**：滿版實景照壓暗＋白色粗體中文大標＋細綠外框。**三項色彩參數全部自動校準到既有 12 面的實測值**，換任何素材都不必重調：平均亮度 **46**（實測 42~50）、飽和度 **38**（實測 32~46）、暖偏 R−B **+30**（實測 +27~+38）
- ⚠️**「溫暖」不能靠疊暖色**：暖提示詞＋暖通道增益＋暖褐混色三層疊起來，實測飽和度衝到 **58~66**（既有面只有 32~46），畫面變單色橘。診斷關鍵＝**R−B 其實正常（+26~+34），爆掉的是飽和度**。飽和與暖偏都繞「通道平均」操作故不影響亮度，可與亮度正規化分開收斂
- ⚠️**校準區與驗證區必須一致**：兩邊都要排除**標題帶**（白字本身就是 255）與**照片框區**（灰框/照片會蓋住底圖）。不排除會量到 70~107 而誤判偏亮
...
- **拼貼演算法＝分帶動態切格＋末格斜切**，關鍵是**用「帶中線」量寬**：末格上方多出的三角形恰補足下方被切掉的，末格淨面積等於同列其他格、不需補償係數；四個末格斜切邊落在同一條安全線上 → 右下邊界是一條與 350 斜邊平行的直線，不是階梯，零廢料
- ⚠️**`ImageFilter.MinFilter` 會先複製邊界像素再侵蝕**，貼齊影像邊界的直線邊完全侵蝕不到（實測：不補邊時上緣/左緣 erode 後仍是 255）。做描邊必須先 `ImageOps.expand(border=w, fill=0)` 再 crop 回來，否則只有圓角有綠邊、直線邊全裸。函式＝`erode()`
- ⚠️**佔位灰必須 `(136,136,136)`**：×1.65 後為 (224,224,224) 是舒服的淺灰；(150,150,150)→(247,247,247) 幾乎看不見、(228,228,228)→純白格子直接消失
- ⚠️**STHeiti 沒有 `▸`**（同既有 `・`/`／` 豆腐地雷），佔位提示字已拿掉箭頭
- 斜切格 #04/#07/#09/#10 上緣 65cm→下緣 22cm，**右下角會被削掉、主體須靠左上**，不要放大合照；程式無法自動偵測人頭被切，**這 4 格必須逐張目視**
- 照片需求：10 張橫式約 5:3，矩形格 ≥605×357px、斜切格 ≥903×357px，建議長邊 ≥1600px。**送餐對象（受助長者）肖像同意須協會逐張確認**
- 母檔 1px=1mm＝25.4dpi 是硬天花板（44cm 格只存 439px 真資料）。若 1:1 預覽不夠實，再本地寫 `build_pdf_hi()` 走 2px/mm，**不要改 `tarp_face_pdf.py`**（12 面共用階段）
