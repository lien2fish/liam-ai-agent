...
| `voice-clone` | 用克隆聲音做旁白／配音；TTS 唸錯字時也看這裡 |
...
### 🕐 全部排程改由 Cloudflare Worker 計時（2026-08-29 完成）

**沒有任何一支 workflow 還有 `schedule`。** 下表的時間全部由 Worker `liam-scheduler`
（`scheduler_worker/`）按時打 `workflow_dispatch`，GitHub 只負責執行。

原因（2026-08-29 實測 20 支的延遲）：**沒有一支準時**，延遲中位 37~689 分、最糟 23 小時；
`ig_comment_reply` 每天只跑掉該跑的 44%；連 `schedule_watchdog` 自己中位都遲到 10 小時
——安全網跟被監控者同一條命。而手動 dispatch 一律秒開 ⇒ **執行沒問題，壞的只有計時**。

- ⛔ **不要把 cron 加回任何一支**——兩邊都留會一天觸發兩次，`daily_post` 會發兩則限動，IG 刪不掉。
- 改時間要同時改 `scheduler_worker/worker.js` 的 `SCHEDULE` 與 `wrangler.toml` 的 `crons`，
  只改一邊會**靜默失效**（對不到 key 就什麼都不做也不報錯）。README 有交叉檢查指令，
  會一併檢查「有沒有時段沒 cron 打得到」「有沒有同一分鐘重複觸發」「有沒有時段沒人稽核」。
- ⛔ **cron 的 `:30` 一律交給 `*/30` 那條**。別的條目再放 `:30` 會在同一分鐘送出兩次
  scheduled 事件——稽核推兩則通知、留言回覆跑兩次。
- 週期性任務寫在 `SCHEDULE` 的值裡：`{ wf, dow: 1 }` 只在 UTC 週一、`{ wf, dom: 1 }` 只在每月 1 日。
- 稽核分兩輪（UTC 02:30 查台灣早上那批、12:30 查下午兩支＋留言回覆）。
  **只查已經過去的時段**——同一輪查還沒到的任務會每天誤報一次。
- `schedule_watchdog` **已退休**：它靠讀 workflow 的 cron 判斷誰該跑，現在讀不到任何 cron。
- 驗證：`curl -H "Authorization: Bearer <NOTIFY_TOKEN>" https://liam-scheduler.lien2fish.workers.dev/selftest`

⚠️ **時間點刻意避開整點與 :00/:15/:30/:45**（2026-08-27 調整）——這個習慣在 Worker 上已不必要
（Cloudflare 準時），但改回整點沒有好處，維持現狀即可。

| Workflow 檔案 | 任務 | 排程 |
|--------------|------|------|
| `daily_post.yml` | IG+FB 每日發文。**2026-08-28 起單雙日交替**：奇數日出解說動畫（`instagram/explainer_video.py`：`zh-TW-YunJheNeural` 男聲旁白＋逐句字幕），偶數日出原本的靜態卡片。**2026-09-06 起插圖改分件生成**：Claude 在 JSON 給 `layers`（背景／中景／前景各一件），各層依 `motion` 各自運動——`water` 緩流＋折射、`seaweed` 底端固定往上擺、`swim` 橫向游動＋尾擺、`drift` 上升、`still` 幾乎不動。⛔ **不會自己移動的東西不准給 swim/drift**，swim 素材必須畫成朝左（畫面由右往左游）。⚠️ **從單張插圖切圖層行不通**（實測單一連通塊佔前景 90.7%），所以要分件就得一開始分開生。三道防呆任一不成立即退回單張：題材沒前後景 Claude 不給 layers／結構不合格／任一件切不出白底。**先試分件、成功就不生單張**，順序顛倒會白花一張錢。⛔ **辨別型題目一律退回靜態圖**（分類或內容關鍵詞命中）——AI 插圖畫不出辨別特徵，配錯圖比沒圖更糟。影片出任何差錯也退回靜態圖並推 LINE。⚠️ 字型要走跨平台偵測，寫死 macOS 路徑在雲端會全滅 | 每天 08:37（2026-08-28 Lien 指定，原 08:07）|
...
## 知識型動畫：兩套工具的分工（2026-09-07 定）

**不是重複的兩套，是兩種不同的內容形態。**

| | `hyperframes_trial/`（HTML＋GSAP） | `tools/knowledge_short.py --insert` |
|---|---|---|
| 產出 | 1 支 30 秒**完整影片**（含旁白、字卡、品牌標記、片尾）| 4~5 段各 4~5 秒的**無字卡圖解片段** |
| 怎麼用 | 直接上傳當 Shorts | **剪進口播影片當 B-roll** |
| 旁白 | edge-tts | 沒有——旁白是 Lien 本人講 |
| 字卡 | 有 | **刻意拿掉**，會跟口播搶注意力 |
| 渲染 | 只能走 GitHub Actions（2015 Air 跑不動）| 本機幾秒出一格 |

```
獨立知識短片  →  HyperFrames   →  直接發 Shorts（不用出鏡）
你的口播影片  →  Python --insert  →  剪進去當示意圖（維持「人在現場講」的識別度）
```

⛔ **不要因為「看起來重複」就砍掉其中一套。**
⛔ **`knowledge_short.py` 的 `vessel-02` 已停用**——它把 AI 生成的「衛星風格」圖
配上「這是從太空拍下來的」旁白。已由 HyperFrames 版（真實 NASA VIIRS 影像）取代。
理由寫在腳本的 `BLOCKED` 裡，四個入口都會擋。
