...
### 六邊形牌換姓名（2026-07-22）
`design/hex_badge_rename.py`：`python3 design/hex_badge_rename.py <來源tiff> <新姓名> [輸出tiff]`。首次應用＝以理事[已移除]牌為模板產出**[已移除]**（TIFF＋.ai 各一份）。**[已移除]仍在任、兩人並存，理事牌 8 位**；只有當初複製出來當來源的 `..._CMYK拷貝.tiff` 工作檔移至 `已撤除_20260722/`。
