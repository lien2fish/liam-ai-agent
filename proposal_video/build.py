...
def parse_script(section):
    body = open(f"{HERE}/content/script.md", encoding="utf-8").read()
...
# 交付三卡與片頭片尾的文字。文案不進公開 repo，一律從 content/scenes.json 讀。
DELIVER = []
FRAME = {}
          ...
          <div class="stage">
            <div class="cardzone">
              <div class="stack" id="intro-c">
                <span class="line l-hook">{FRAME["intro_hook"][0]}</span>
                <span class="line l-hook gold">{FRAME["intro_hook"][1]}</span>
              </div>
            </div>
            <div class="tag" id="intro-t">{FRAME["intro_tag"]}</div>
          </div>
        </div>
      </section>
          ...
          <div class="stage">
            <div class="cardzone">
              <div class="stack" id="outro-c">
                <span class="line l-hook gold">{FRAME["outro_hook"]}</span>
                <span class="line l-small">{FRAME["outro_sub"]}</span>
              </div>
            </div>
          </div>
        </div>
...
def main():
    scenes_all = json.load(open(f"{HERE}/content/scenes.json", encoding="utf-8"))
    global DELIVER, FRAME
    DELIVER = [tuple(x) for x in scenes_all["deliver"]]
    FRAME = scenes_all["frame"]
